...
class Tracing extends _instrumentation.SdkObject {
  ...
  async start(options) {
    ...
    this._writeChain = _fs.default.promises.mkdir(resourcesDir, {
      recursive: true
    }).then(() => _fs.default.promises.writeFile(networkFile.file, ''));
  ...
  async startChunk(options = {}) {
    ...
    this._appendTraceOperation(async () => {
      await (0, _fileUtils.mkdirIfNeeded)(traceFile.file);
      const event = {
        ...this._contextCreatedEvent,
        title: options.title,
        wallTime: Date.now()
  ...
  _changeTraceName(state, name) {
    const {
      traceFile: oldTraceFile,
      networkFile: oldNetworkFile
    } = state;
    ...
    this._appendTraceOperation(async () => {
      ...
      await _fs.default.promises.copyFile(oldNetworkFile.file, networkFile.file);
    });
  }
  async stop() {
  ...
  async stopChunk(params) {
    ...
    this._appendTraceOperation(async () => {
      ...
      await flushTraceFile(oldNetworkFile);
      await _fs.default.promises.copyFile(oldNetworkFile.file, newNetworkFile);
      if (params.mode === 'entries') result = {
        entries
      };else result = {
        artifact: await this._exportZip(entries, traceFile.file + '.zip').catch(() => undefined)
      };
    });
    try {
      await this._writeChain;
  ...
  _exportZip(entries, zipFileName) {
