...
class Frame extends _instrumentation.SdkObject {
  ...
  constructor(page, id, parentFrame) {
    ...
    this._detachedRace = new _utils.ScopedRace();
  ...
  async raceNavigationAction(progress, options, action) {
    return _utils.ScopedRace.raceMultiple([this._detachedRace, this._page._disconnectedRace, this._page._crashedRace], action().catch(e => {
  ...
  async retryWithProgressAndTimeouts(progress, timeouts, action) {
    ...
    while (progress.isRunning()) {
      ...
      if (timeout) {
        ...
        await _utils.ScopedRace.raceMultiple([this._page._disconnectedRace, this._page._crashedRace, this._detachedRace], actionPromise);
      }
      progress.throwIfAborted();
      try {
  ...
  _contextDestroyed(context) {
    ...
    if (this._detachedRace.isDone()) return;
  ...
  _startNetworkIdleTimer() {
    ...
    if (this._firedLifecycleEvents.has('networkidle') || this._detachedRace.isDone()) return;
...
class SignalBarrier {
  ...
  async addFrameNavigation(frame) {
    ...
    await _utils.ScopedRace.raceMultiple([frame._page._disconnectedRace, frame._page._crashedRace, frame._detachedRace], waiter.promise).catch(() => {});
    waiter.dispose();
    this.release();
  }
  retain() {
